...
| | 1ère application d'une nouvelle baseline Generic ETCS **majeure** | TSI1 | Oui | CSM4 | Oui | CEN3 | Oui | CAT4 |
...
### 2.6 Liens entre catégories CSM, CEN, TSI et Interlocking

#### 2.6.1 Types de changements applicables à l'Interlocking PLP (dans SFU uniquement)

| Type de changement | Sous-type / Cas d'application | Catégories TSI possibles | NoBo? | Catégories CSM possibles | AsBo? | Catégories CEN possibles | ISA? | CAT IL.PLP |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **(TYP2 - SC)** Substitution d'un produit certifié existant | Remplacement des équipements défectueux lors des activités de maintenance prises en charge par WIT/PTR | TSI-1, TSI0 | Non | CSM1 | Non | CEN1 | Non | CAT1 |
| **(TYP13 - DMC)** Déploiement de la modification d'un système de référence certifié | L’un des critères ci-dessous est rempli :<br>(1) Utilisation pour la première fois dans un projet du BSL et du PTR génériques SM400T mis à niveau.<br>(2) Impact sur les principes fondamentaux de signalisation.<br>(3) Impact sur les interfaces IL‑PLP vers les produits périphériques et/ou les nouveaux produits périphériques tels que AW, ACAT, CV.SK, EBP, RBC, etc. |  TSI-1, TSI1 (Uniquement en cas de changement des constituants d’interopérabilité faisant partie d’un projet PLP, tels que ACAT, CV.SK ou tout autre projet PLP auquel les exigences STI s’appliquent) | Oui (si TSI1) | CSM2, CSM3, CSM4 | Oui (si CSM3/4) | CEN3 | Oui | CAT3 |
|  | Changement significatif du BSL générique IL.PLP nécessitant une nouvelle ligne pilote. Cela inclut un nouveau produit cœur PLP remplaçant le CIXL, le Smart GW ou le TFM. | TSI-1, TSI1 (Uniquement en cas de changement des constituants d’interopérabilité faisant partie d’un projet PLP, tels que ACAT, CV.SK ou tout autre projet PLP auquel les exigences STI s’appliquent) | Oui (si TSI1) | CSM3, CSM4 | Oui | CEN3 | Oui | CAT4 |
| **(TYP14 - DC)** Déploiement d'un système de référence certifié et inchangé | Tous les critères ci-dessous sont remplis :  <br>(1) Le BSL et le PTR génériques SM400T sont déjà utilisés dans un projet. Aucun changement dans le logiciel ou le matériel de base. <br>(2) Aucun impact sur les principes fondamentaux de signalisation. <br>(3) Aucun impact sur les interfaces IL‑PLP vers les produits périphériques et/ou les nouveaux produits périphériques tels que AW, ACAT, CV.SK, EBP, RBC, etc. | TSI-1 | Non | CSM2 | Non | CEN2+ (I-B103S safety statement) | Non | CAT2 |
| **(TYP15 - DN)** Création et déploiement d'un nouveau système | - | TSI-1, TSI1 (Only if change of Interoperability constituents part of a PLP project such as ACAT, CV.SK or any other PLP project where STI requirements apply) | Oui (si TSI1) | CSM4 | Oui | CEN3 | Oui | CAT4 |

#### 2.6.1 Types de changements applicables à l'Interlocking SIW (dans SFU uniquement)

| Type de changement | Sous-type / Cas d'application | Catégories TSI possibles | NoBo? | Catégories CSM possibles | AsBo? | Catégories CEN possibles | ISA? | CAT IL.PLP |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **(TYP2 - SC)** Substitution d'un produit certifié existant | Remplacement des équipements défectueux lors des activités de maintenance prises en charge par WIT/PTR | TSI-1, TSI0 | Non | CSM1 | Non | CEN1 | Non | CAT1 |
| **(TYP13 - DMC)** Déploiement de la modification d'un système de référence certifié | Changement dans le GASC IL.SIW sans nécessité de ligne pilote ni de première mise en œuvre spécifique au projet. |  TSI-1, TSI1 (Uniquement en cas de changement de produit TVP) | Oui (si TSI1) | CSM2, CSM3, CSM4 | Oui (si CSM3/4) | CEN3 | Oui | CAT3 |
|  | Changement dans le GASC IL.SIW avec nécessité d’une nouvelle ligne pilote ou d’une première mise en œuvre spécifique au projet. Cela inclut un nouveau produit, un nouvel environnement, un changement fonctionnel majeur ou un changement de principe majeur. | TSI-1, TSI1  (Uniquement en cas de changement de produit TVP) | Oui (si TSI1) | CSM3, CSM4 | Oui | CEN3 | Oui | CAT4 |
| **(TYP14 - DC)** Déploiement d'un système de référence certifié et inchangé | « Basic design » = pour IL, conception générique Infrabel et THV (GASC). <br> « Technical file » = pour IL, ensemble complet de la documentation projet (SASC) appuyé par la conception générique (GASC). <br> Catégories de rework identifiées dans le document PSI (PM, IL.SIW, z) Rework Change impact NLS MDL 1.1 E (*). | TSI-1, TSI-1 (Uniquement en cas de modification des limites du projet?) | Oui (si TSI1) | CSM2 | Non | CEN2, CEN3 | Oui (si CEN3) | CAT2 |
| **(TYP15 - DN)** Création et déploiement d'un nouveau système | - |TSI-1, TSI1  (Uniquement en cas de changement de produit TVP) | Oui (si TSI1) | CSM4 | Oui | CEN3 | Oui | CAT4 |
